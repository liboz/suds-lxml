...
def test_choice_parameter_implementation_inconsistencies():
    ...
    """
    def client(x, y):
        return testutils.lxmlclient_from_wsdl(testutils.wsdl(x, input=y))
...
def test_empty_invalid_WSDL(monkeypatch):
    ...
    e = pytest.raises(xml.sax.SAXParseException, testutils.lxmlclient_from_wsdl,
        wsdl)
    try:
...
def test_parameter_referencing_missing_element(monkeypatch):
    ...
    monkeypatch.delitem(locals(), "e", False)
    e = pytest.raises(suds.TypeNotFound, testutils.lxmlclient_from_wsdl, wsdl)
    try:
...
def test_restrictions():
    client_unnamed = testutils.lxmlclient_from_wsdl(testutils.wsdl("""\
      <xsd:element name="Elemento">
        <xsd:simpleType>
    ...
    client_named = testutils.lxmlclient_from_wsdl(testutils.wsdl("""\
    ...
    client_twice_restricted = testutils.lxmlclient_from_wsdl(testutils.wsdl("""\
